...
// for debugging
static void inline eth_write_verbose(struct net_local *priv, size_t addr, int data)
{
  volatile uint64_t *eth_base = (volatile uint64_t *)(priv->ioaddr);
#ifdef VERBOSE
  printk("eth_write(0x%lX, 0x%X);\n", addr, data);
#endif
  eth_base[addr >> 3] = data;
}
...
// for debugging
static volatile inline int eth_read_verbose(struct net_local *priv, size_t addr)
{
  volatile uint64_t *eth_base = (volatile uint64_t *)(priv->ioaddr);
  int rslt = eth_base[addr >> 3];
#ifdef VERBOSE
  printk("eth_read(0x%lX, 0x%X);\n", addr, rslt);
#endif
  return rslt;
}
...
static int lowrisc_mdio_read(struct mii_bus *bus, int phy_id, int reg)
{
  return 0;
}

static int lowrisc_mdio_write(struct mii_bus *bus, int phy_id, int reg, u16 val)
{
  return 0;
}
...
static int lowrisc_mii_init(struct net_device *ndev)
{
	...
	snprintf(new_bus->id, MII_BUS_ID_SIZE, "lowrisc-0");
        new_bus->name = "LowRISC",
        new_bus->read = lowrisc_mdio_read;
        new_bus->write = lowrisc_mdio_write;
...
static void lowrisc_self_test(struct net_device *ndev,
                           struct ethtool_test *etest,
                           u64 *buf)
{
        struct net_local *priv = netdev_priv(ndev);

        memset(buf, 0, sizeof(u64) * priv->num_tests);
}
...
static int lowrisc_open(struct net_device *ndev)
{
  ...
  retval = request_irq(priv->irq, lowrisc_ether_isr, IRQF_SHARED, ndev->name, priv);
  if (retval) {
